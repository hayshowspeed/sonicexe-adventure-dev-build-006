...
function levelcleanup () {
    for (let value2 of sprites.allOfKind(SpriteKind.Badnik)) {
        value2.destroy()
    }
    for (let value3 of sprites.allOfKind(SpriteKind.CollectibleRing)) {
        value3.destroy()
    }
    for (let value4 of sprites.allOfKind(SpriteKind.Spring)) {
        value4.destroy()
    }
    for (let value5 of sprites.allOfKind(SpriteKind.Goal)) {
        value5.destroy()
    }
}
function loadlevel () {
...
function placeplayer () {
    for (let value6 of tiles.getTilesByType(assets.tile`myTile4`)) {
        tiles.placeOnTile(mySprite, value6)
...
function placesprings () {
    for (let value7 of tiles.getTilesByType(assets.tile`myTile9`)) {
        springobject = sprites.create(img`
            ...
            . . . . . . c c c c . . . . . . 
            `, SpriteKind.Spring)
        tiles.placeOnTile(springobject, value7)
        tiles.setTileAt(value7, assets.tile`transparency16`)
    }
    for (let value8 of tiles.getTilesByType(assets.tile`myTile11`)) {
        springobject2 = sprites.create(img`
            ...
            c c c . . . . . . . . . . . . . 
            `, SpriteKind.Spring)
        tiles.placeOnTile(springobject2, value8)
    ...
    for (let value9 of tiles.getTilesByType(assets.tile`myTile12`)) {
        springobject3 = sprites.create(img`
            ...
            . . . . . . . . . . . . . c c c 
            `, SpriteKind.Spring)
        tiles.placeOnTile(springobject3, value9)
...
function placerings () {
    for (let value10 of tiles.getTilesByType(assets.tile`myTile5`)) {
        ...
        tiles.placeOnTile(ringobject, value10)
...
function placeenemy () {
    for (let value11 of tiles.getTilesByType(assets.tile`myTile13`)) {
        ...
        tiles.placeOnTile(badnikobject, value11)
...
game.onUpdate(function () {
    for (let value12 of sprites.allOfKind(SpriteKind.Badnik)) {
        if (Math.abs(value12.x - mySprite.x) < 60) {
            if (value12.x - mySprite.x < -5) {
                value12.vx = 25
            } else if (value12.x - mySprite.x > 5) {
                value12.vx = -25
            }
            if (value12.y - mySprite.y < -5) {
                value12.ay += 9.8 * pixelstometers
            } else if (value12.y - mySprite.y > 5) {
                value12.ay += 9.8 * pixelstometers
            }
        } else {
            value12.vy = 0
...
game.onUpdateInterval(2000, function () {
    sonicexe = sprites.createProjectileFromSide(img`
        ..fffffffff..ff......
        .f888888888ff8f......
        ..f888fd888888f......
        ...f88fdd88888f......
        ...
        .....ffffffffffffff..
        `, 20, 20)
    sonicexe.setKind(SpriteKind.Enemy)
    animation.runImageAnimation(
    sonicexe,
    assets.animation`sonicexee`,
    500,
    true
    )
    animation.runImageAnimation(
    ...
    mySprite2 = sprites.create(assets.image`sonicexe menu cancel one`, SpriteKind.Food)
    music.play(music.createSong(hex``), music.PlaybackMode.InBackground)
    music.play(music.createSoundEffect(WaveShape.Triangle, 2387, 2892, 255, 239, 500, SoundExpressionEffect.None, InterpolationCurve.Linear), music.PlaybackMode.InBackground)
    music.play(music.stringPlayable("C G E F E A E G ", 140), music.PlaybackMode.UntilDone)
})
